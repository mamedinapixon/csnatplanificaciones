        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="flex items-center justify-between no-print mb-4">
                <x-pixonui.heading.h1>
                    <small class="font-light">{{ $periodo_lectivo }} | {{ $carrera }}</small><br>
                    {{ $asigantura }}
                </x-pixonui.heading.h1>
                <div class="flex items-center space-x-2">
                    <button type="button"
                            onclick="window.print()"
                            class="inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Imprimir
                    </button>
                    <a href="{{ route('planificacion.pdf', $planificacion) }}"
                       class="inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-md shadow hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                        Descargar PDF
                    </a>
                </div>
            </div>
                ...
                <div class="print-p-0 p-6 space-y-2 bg-white border-b border-gray-200 sm:px-20">
                    ...
                    @if (!empty($planificacion->cv_externo))
                        ...
                        @foreach ($planificacion->cv_externo as $rutaCv)
                            <div class="flex justify-start items-center no-print">
                                <img src="{{ asset('img/icon-pdf.png') }}">
                                <div><a target="_blank" href="{{ asset($rutaCv) }}">Ver cv docente externo</a></div>
                            </div>
                            <div class="flex justify-start items-center print">
                                <img src="{{ asset('img/icon-pdf.png') }}">
                                <a target="_blank" href="{{ asset($rutaCv) }}">{{ config('app.test_url') . '' . asset($rutaCv) }}</a>
                            </div>
                        @endforeach
                    @endif
                </div>
